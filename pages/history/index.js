            ...
            <div className="mt-6 space-y-4">
              {historyData.map((item, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="mx-auto flex items-center justify-between rounded-xl border border-gray-200 bg-white p-4 pl-5 pr-10 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white-light"
                  >
                    <div className="flex flex-col">
                      ...
                      {/* Status */}
                      <span className={`mt-1 text-sm ${status.color}`}>
                        {status.text}
                      </span>
                    </div>
        ...
        <>
          {showReceipt && (
            <div className="mb-2 mt-10">
              <button
                ...
                className="flex items-center gap-2 font-bold text-green-500 hover:text-green-700 dark:text-white-light"
              >
                {/* Back arrow icon */}
                <svg
                  className="h-4 w-4"
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 text-center text-gray-500 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white-light">
            {receiptData.map((field, index) => (
              <div
                key={`receipt${index}`}
                className="flex justify-between border-b border-gray-200 py-2 dark:border-gray-700 dark:text-white-light"
              >
                <p className="text-sm font-semibold text-gray-700 dark:text-white-light">
                  {field.label}
                </p>
