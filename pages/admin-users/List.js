...
const List = () => {
  ...
  const getProfiles = async () => {
    try {
      ...
      let querySnapshot;
      if (search) {
        const q = query(profileCol, where('firstName', '==', search));
        querySnapshot = await getDocs(q);
      } else {
        querySnapshot = await getDocs(profileCol);
      }
      ...
      setTotalCount(total);
      setItems(profilesArray);
      setInitialRecords(profilesArray);
      setIsLoading(false);
    } catch (error) {
