...
const List = () => {
  ...
  const getFileAppeals = async () => {
    try {
      setIsLoading(true);
      const fileAppealCol = collection(db, TABLE_NAME.fileappeal);
      const fileAppealSnapshot = await getDocs(fileAppealCol);
      let querySnapshot;
      if (search) {
        const q = query(fileAppealSnapshot, where('email', '==', search));
        querySnapshot = await getDocs(q);
      } else {
        querySnapshot = await getDocs(fileAppealCol);
      }
      const fileAppealsArray = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));

      const total = fileAppealSnapshot.size;
      setTotalCount(total);
      setItems(fileAppealsArray);
      setInitialRecords(fileAppealsArray);
      setIsLoading(false);
    } catch (error) {
      ...
      <div className="invoice-table">
        <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl">File Appeals List</h1>
          </div>
