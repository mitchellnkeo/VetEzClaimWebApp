...
const List = () => {
  ...
  const getFileAppeals = async () => {
    try {
      setIsLoading(true);
      const fileFormsCol = collection(db, TABLE_NAME.fillform);
      const fileFormsSnapshot = await getDocs(fileFormsCol);
      let querySnapshot;
      if (search) {
        const q = query(fileFormsSnapshot, where('email', '==', search));
        querySnapshot = await getDocs(q);
      } else {
        querySnapshot = await getDocs(fileFormsCol);
      }
      const fileFormsArray = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));

      const total = fileFormsSnapshot.size;
      setTotalCount(total);
      setItems(fileFormsArray);
      setInitialRecords(fileFormsArray);
      setIsLoading(false);
    } catch (error) {
      ...
      <div className="invoice-table">
        <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl">Fill Forms List</h1>
          </div>
